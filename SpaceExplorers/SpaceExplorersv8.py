...
def detectCollisionsWithMainShip(inbulnerable,meteor1,meteor2,meteor3,meteor4,meteor5,main_ship,main_bullet,shipSprites,meteorSprites,lifes,gameOver,reloadBullet1,bullet_cont,destroyedMeteorCount,block,enemyMartianShip,enemyMartian_bullet,enemyMartianShipLifes,enemyMartianShip2,enemyMartian_bullet2,enemyMartianShipLifes2):
    ...
    if impacts_lists_main_bullet_meteor1 and main_bullet.rect.x<950:
        meteorDestroy()
        meteor1.rect.x=1300
    ...
    if impacts_lists_main_bullet_meteor2 and main_bullet.rect.x<950:
        meteorDestroy()
        meteor2.rect.x=1300
    ...
    if impacts_lists_main_bullet_meteor3 and main_bullet.rect.x<950:
        meteorDestroy()
        meteor3.rect.x=1300
    ...
    if impacts_lists_main_bullet_meteor4 and main_bullet.rect.x<950:
        meteorDestroy()
        meteor4.rect.x=1300
    ...
    if impacts_lists_main_bullet_meteor5 and main_bullet.rect.x<950:
        meteorDestroy()
        meteor5.rect.x=1300
...
def makeShip(shipSprites, x, y):
    ...
    ship.image = pygame.Surface([34, 25])
...
def gestionarEventos(inbulnerable,main_ship,main_bullet,main_bullet_run,isRunning,bullet_cont,block):
    ...
    for evento in pygame.event.get(): # El usuario realizó alguna acción 
        ...
        if evento.type == pygame.QUIT: # Si el usuario hizo click en salir
            ...
            if evento.key == pygame.K_SPACE and main_bullet.rect.x>1100 and main_ship.rect.y>=398 and bullet_cont>0 and block and not(inbulnerable):
    ...
    return isRunning,main_bullet_run,bullet_cont
...
while isRunning:
    ...
    if menuStart:
        ...
        if menuHowToPlay:
            ...
            isRunning,main_bullet_run,bullet_cont = gestionarEventos(inbulnerable,main_ship,main_bullet,main_bullet_run,isRunning,bullet_cont,block)

            if main_bullet_run and main_bullet.rect.x<1200:
                main_bullet.rect.x+=10


            planetsSprites.draw(screen)#al fondo   
            meteorSprites.draw(screen)#delante
            bulletReloadSprites.draw(screen)#delante de meteoritos
            ...
            if enemyMartian_bullet_run2 and enemyMartian_bullet2.rect.x>-500:
                enemyMartian_bullet2.rect.x-=5
                enemyMartian_bullet2.rect.y=enemyMartianShip2.rect.y+3
                print("dispara marciano 222")
